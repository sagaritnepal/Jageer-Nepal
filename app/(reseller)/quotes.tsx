...
function ResellerQuoteCard({ item }: { item: ProductQuote }) {
  const { data: product } = useSupabaseRow('products', item.product_id);
  const { data: client } = useSupabaseRow('profiles', item.client_id);

  return (
    <Pressable
      onPress={() => router.push(`/(reseller)/quote/${item.id}`)}
    ...
    >
      ...
      <Text className="mt-1 text-xs text-gray-500">
        {client?.full_name ?? 'Customer'} · Qty: {item.quantity}
      </Text>
...
export default function ResellerQuotes() {
  ...
  return (
    <View className="flex-1 bg-gray-50 px-6 pt-16">
      <Text className="mb-6 text-2xl font-bold text-gray-900">Quotes</Text>
      ...
      <FlatList data={filtered} keyExtractor={(item) => item.id} renderItem={({ item }) => <ResellerQuoteCard item={item} />} />
    </View>
  );
}
